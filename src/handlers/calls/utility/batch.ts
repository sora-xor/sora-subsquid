import { addCallsToHistoryElement, createCallHistoryElement } from '../../../utils/history'
...
type Version = Exclude<keyof typeof generatedCalls.utility.batch, 'name'>
type Calls = ReturnType<typeof getCallData<typeof generatedCalls.utility.batch, 'exclude', []>>['calls']
...
function mapCallsForAllVersions(
	ctx: BlockContext,
	call: Call<'Utility.batch'>,
	historyElement: HistoryElement,
): HistoryElementCall[] {
	let calls: HistoryElementCall[] | null = null
	const versions = Object.keys(generatedCalls.utility.batch).filter(v => v !== 'name') as Version[]
	versions.forEach((version: Version) => {
		if (generatedCalls.utility.batch[version].is(call)) {
			calls = mapCalls(
				ctx,
				{
					version,
					calls: generatedCalls.utility.batch[version].decode(call).calls,
				} as BatchCalls,
				historyElement,
			)
		}
...
export async function utilityBatchCallHandler(ctx: BlockContext, call: Call<'Utility.batch'>): Promise<void> {
