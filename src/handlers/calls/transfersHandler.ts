...
export async function handlerTransfers(ctx: Context, block: Block, call: CallEntity): Promise<void> {

    if (call.kind !== 'call' || call.name !== 'Assets.transfer') return
    ...
    ctx.log.debug(`===== Saved transfer with ${call.extrinsic.hash} txid =====`)
