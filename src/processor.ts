...
processor.run(new TypeormDatabase(), async (ctx) => {
    ...
    for (let block of context.blocks) {
		const blockContext = { ...context, block }
		...
		const sort = (items: Items): Items => {
			...
			items.forEach(item => {
				...
				const lastGroupExtrinsicHash = lastGroup?.[0].kind === 'call' ? lastGroup[0].extrinsic.hash : lastGroup?.[0].event.extrinsic?.hash
		...
		for (let item of sort(block.items)) {
            ...
            if (item.kind === 'call' && item.name === item.extrinsic.call.name) {
				...
				if (item.name === 'Staking.submit_election_solution_unsigned') await stakingSubmitElectionSolutionUnsignedCallHandler(blockContext, item)
