...
// Make sure to add "as const" after the versions array to properly narrow the entity object
export function getEntityData<T extends VersionedObject, V extends readonly number[]>(ctx: Context, block: Block, entity: T, { kind, name }: { kind: 'call' | 'event' | 'storage', name: string }, versions?: V): Exclude<NarrowVersions<T, V>[keyof NarrowVersions<T, V>], boolean> {
	versions = versions ?? getAllVersions(entity) as V
