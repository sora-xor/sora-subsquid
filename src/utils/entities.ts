...
type NarrowVersions<T, V extends readonly string[]> = {
    [K in Extract<keyof T, `isV${string}` | `asV${string}`> as K extends `isV${infer R}` ? R extends `${V[number]}` ? K : never : K extends `asV${infer R}` ? R extends `${V[number]}` ? K : never : never]: T[K]
}
...
function getAllVersions(obj: any): readonly string[] {
	return getVersionedObjectKeys(obj).filter(key => key.startsWith('isV')).map(key => key.slice(3))
}
...
type ExcludeVersions<T, V extends readonly string[]> = V extends []
  ? T
  : {
      [K in Extract<keyof T, `isV${string}` | `asV${string}`> as K extends `isV${infer R}` ? R extends `${V[number]}` ? never : K : K extends `asV${infer R}` ? R extends `${V[number]}` ? never : K : never]: T[K];
    };
